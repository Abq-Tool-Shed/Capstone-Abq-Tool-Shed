...
export async function selectPartialProfileByProfileId(profileId: string) : Promise<PartialProfile | null> {
    try {
        ...
        const mysqlQuery : string = 'SELECT BIN_TO_UUID(profileId) as profileId, profileHandle, profileName, profilePhoneNumber, profileEmail, profileProfileImage FROM profile WHERE profileId = UUID_TO_BIN(:profileId)'
