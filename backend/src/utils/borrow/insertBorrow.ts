...
export async function insertBorrow(borrow: Borrow) : Promise<string> {
    try {
        ...
        const mySqlQuery = "INSERT INTO borrow(borrowId, borrowProfileId, borrowToolId, borrowCompleted, borrowDateTime) VALUES(UUID_TO_BIN(UUID()), UUID_TO_BIN(:borrowProfileId), UUID_TO_BIN(:borrowToolId), :borrowCompleted, NOW())";
