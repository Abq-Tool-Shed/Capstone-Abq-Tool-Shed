...
export async function selectToolByToolId(toolId: string) : Promise<Tool|null> {
    try {
        ...
        const mySqlQuery = " SELECT BIN_TO_UUID(toolId) AS toolId, BIN_TO_UUID (toolProfileId) AS toolProfileId, BIN_TO_UUID(toolCategoryId) as toolCategoryId, toolDescription, toolPostDate from tool where toolId = UUID_TO_BIN(:toolId)"
